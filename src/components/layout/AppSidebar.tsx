...
export function AppSidebar({ activeView, onViewChange, user }: { activeView: string, onViewChange: (view: string) => void, user: any }) {
  ...
  return (
    <Sidebar variant="sidebar" collapsible="icon" className="bg-black/80 border-r border-white/5 backdrop-blur-lg">
      ...
      <SidebarContent className="px-3 space-y-4 py-4">
              ...
              {navItems.map((item) => (
                <SidebarMenuItem key={item.id}>
                  <SidebarMenuButton
                    ...
                    className={`px-3 py-2.5 rounded-lg transition-all duration-200 text-xs h-10 w-full justify-start gap-3 ${
                      activeView === item.id
                  ...
                  >
                    <item.icon className={`size-4 shrink-0 ${activeView === item.id ? "text-primary" : ""}`} />
        ...
        <SidebarGroup className="border-t border-white/10 pt-4">
          <SidebarGroupLabel className="px-3 text-[7px] uppercase tracking-widest opacity-40 font-bold mb-3">Management</SidebarGroupLabel>
          <SidebarGroupContent>
            <SidebarMenu className="space-y-1">
      ...
      <SidebarFooter className="border-t border-white/10 bg-gradient-to-t from-black/60 to-transparent p-4">
        <div className="space-y-3">
          <div className="flex items-center gap-3">
            <Avatar className="h-9 w-9 border border-primary/30 shrink-0 shadow-lg shadow-primary/10">
            ...
            <Button variant="ghost" size="icon" className="size-8 shrink-0 hover:bg-primary/10 text-muted-foreground hover:text-primary transition-all" onClick={handleSignOut}>
              <LogOut className="size-4" />
            </Button>
          </div>
          <div className="h-px bg-gradient-to-r from-white/5 via-primary/10 to-white/5" />
          <p className="text-[6px] text-muted-foreground/50 uppercase tracking-[0.1em] font-bold px-1">v1.0 Enterprise</p>
        </div>
      </SidebarFooter>
    </Sidebar>
  );
