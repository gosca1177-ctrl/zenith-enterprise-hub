...
export function BusinessMarketplace() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const data = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as BusinessListing));
      setListings(data);
      setIsLoading(false);
    });
    return () => unsubscribe();
      ...
      <CardContent className="flex-1 pb-6">
        <div className="grid grid-cols-2 gap-6 mt-4">
          <div className="space-y-1.5 border-l border-white/5 pl-3">
            ...
            <p className="text-lg font-serif">€{listing.monthlyRevenue.toLocaleString()}</p>
          </div>
          <div className="space-y-1.5 border-l border-primary/20 pl-3">
            ...
            <p className="text-lg font-serif text-emerald-500">€{listing.monthlyProfit.toLocaleString()}</p>
          </div>
        </div>
      </CardContent>
